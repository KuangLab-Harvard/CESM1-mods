...
subroutine crm        (lchnk, icol, icopy, &
...
! hm 8/31/11 new output for gcm-grid and time-step avg process rates
                       ...
                       cltot, clhgh, clmed, cllow, cld_loc, cldtop, &
                       gicewp, gliqwp, &
...
!  Input:
         ...
         integer, intent(in) :: icopy    ! crm copy identifier
...
! hm 8/31/11 new output, gcm-grid and time step-avg
         ...
         real(r8), intent(out) :: cld_loc(plev)  ! cloud fraction
...
!--------------------------------------------------
     ...
     if(doprecip) call precip_init()
        ...
        iseed = gcolindex(icol)**2 + icopy**3 ! TODO: idk if I need to do this
...
!           crm_qs(l) = crm_qs(l) + 0.     ! temporerary solution
        ...
        cld_loc = min(1._r8,cld_loc/float(nstop)*factor_xy)
