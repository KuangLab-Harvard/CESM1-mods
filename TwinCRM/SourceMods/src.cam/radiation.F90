...
!-----------------------------------------------------------------------
    ...
    cldloc_idx = pbuf_get_index('CLD_LOC')
    ...
    call add_default ('SOLIN   ', 1, ' ') ! used in restart
    ...
    cicewp_idx = pbuf_get_index('CICEWP') ! has ncopy dims
    cliqwp_idx = pbuf_get_index('CLIQWP') ! has ncopy dims
...
!===============================================================================
  ...
  subroutine radiation_tend(state,ptend, pbuf, &
    ...
    integer, target :: nmxrgn_loc(pcols,crm_ncopy)       ! pbuf pointer to Number of maximally overlapped regions - used for SPCAM
    real(r8),target :: pmxrgn_loc(pcols,crm_ncopy,pverp) ! Maximum values of pressure for each - used for SPCAM
                                               !    maximally overlapped region.
                                               !    0->pmxrgn(i,1) is range of pressure for
                                               !    1st region,pmxrgn(i,1)->pmxrgn(i,2) for
                                               !    2nd region, etc

    real(r8),pointer :: emis(:,:,:)              ! Cloud longwave emissivity - calculated for CRM
    ...
    real(r8),target :: emis_loc(pcols,crm_ncopy,pver)    ! Cloud longwave emissivity - used for SPCAM
    real(r8),target :: cldtau_loc(pcols,crm_ncopy,pver)  ! Cloud longwave optical depth - used for SPCAM
    real(r8),target :: cicewp_loc(pcols,crm_ncopy,pver)  ! in-cloud cloud ice water path - used for SPCAM
    real(r8),target :: cliqwp_loc(pcols,crm_ncopy,pver)  ! in-cloud cloud liquid water path - used for SPCAM
    ...
    real(r8), pointer, dimension(:,:,:) :: cld_loc  ! cloud fraction with both CRM copies

    real(r8), pointer :: t_rad (:,:,:,:,:) ! rad temperuture - have icopy dims
    ...
    real(r8) trad(pcols,crm_ncopy,pver)
    real(r8) qvrad(pcols,crm_ncopy,pver)
    real(r8) cldn(pcols,pver)
    real(r8) cldn_loc(pcols, crm_ncopy, pver)
    real(r8) fice(pcols,crm_ncopy,pver)
    ...
    real(r8) cld_crm(pcols,crm_ncopy, crm_nx, crm_ny, crm_nz)
    real(r8) cliqwp_crm(pcols,crm_ncopy, crm_nx, crm_ny, crm_nz)
    real(r8) cicewp_crm(pcols,crm_ncopy, crm_nx, crm_ny, crm_nz)
    real(r8) rel_crm(pcols,crm_ncopy, crm_nx, crm_ny, crm_nz)
    real(r8) rei_crm(pcols,crm_ncopy, crm_nx, crm_ny, crm_nz)
    real(r8) emis_crm(pcols,crm_ncopy, crm_nx, crm_ny, crm_nz)
    real(r8) qrl_crm(pcols,crm_ncopy, crm_nx, crm_ny, crm_nz)
    real(r8) qrs_crm(pcols,crm_ncopy, crm_nx, crm_ny, crm_nz)
    ...
    real(r8) qrs_m(pcols,crm_ncopy,pver)
    real(r8) qrl_m(pcols,crm_ncopy,pver)
    ...
    real(r8) qrsn(pcols,pver) ! for saving buffer copy
    real(r8) qrln(pcols,pver)
    logical :: first_column
    logical :: last_column_crm1
    logical :: last_column_crm2
...
!----------------------------------------------------------------------
    ...
    first_column = .false.
    last_column_crm1 = .false.
    last_column_crm2 = .false.
    ...
    if (use_SPCAM) then 
       ...
       call pbuf_get_field(pbuf, crm_t_rad_idx,  t_rad) ! has ncopy dimensions
       call pbuf_get_field(pbuf, crm_qc_rad_idx, qc_rad) ! has ncopy dimensions
       call pbuf_get_field(pbuf, crm_qi_rad_idx, qi_rad) ! has ncopy dimensions
       call pbuf_get_field(pbuf, crm_qv_rad_idx, qv_rad) ! has ncopy dimensions
       call pbuf_get_field(pbuf, crm_qrad_idx,   crm_qrad) ! has ncopy dimensions
 
       call pbuf_get_field(pbuf, cldloc_idx,     cld_loc) ! has ncopy dimensions
    endif
    ...
    ! save qrs/qrl from buffer so that we can use it again for the second CRM run
    qrsn(1:ncol,1:pver) = 0.
    qrln(1:ncol,1:pver) = 0.
    qrsn = qrs
    qrln = qrl
    ...
    if (dosw .or. dolw) then
       ...
       if (use_SPCAM) then
          ...
          cicewp => cicewp_loc ! pointer assignment
          cliqwp => cliqwp_loc
          emis   => emis_loc
          cldtau => cldtau_loc
          pmxrgn => pmxrgn_loc
       ...
       if (use_SPCAM) then 
          ...
          cldn_loc(1:ncol,:,1:pver) = 0.
          cicewp(1:ncol,:,1:pver) = 0.
          cliqwp(1:ncol,:,1:pver) = 0.
          do icopy=1,crm_ncopy
            trad(:ncol,icopy,:)  = state%t(:ncol,:)
            qvrad(:ncol,icopy,:) = state%q(:ncol,:,1)  ! is this ok, or should sp_hum be used? +++mhwang
          end do

          factor_xy = 1./dble(crm_nx*crm_ny)

          cldn = cld  ! save to restore later
          cldn_loc = cld_loc ! save to restore later
       end if

       do icopy=crm_ncopy,1,-1 ! do crm 2 first so crm1 vars are saved at end.
         solin_m =0.
         fsntoa_m =0.
         fsutoa_m =0.
         fsntoac_m=0.
         ...
         !call pbuf_get_field(pbuf, qrs_idx,qrs)
         !call pbuf_get_field(pbuf, qrl_idx,qrl)
         qrs = qrsn ! maybe not required bc qrs reloaded in radsw code
         qrl = qrln
         ...
         do ii=1,crm_nx 
           ...
           if (use_SPCAM) then
              ...
              last_column_crm1 = ii.eq.crm_nx.and.jj.eq.crm_ny.and.icopy.eq.1
              last_column_crm2 = ii.eq.crm_nx.and.jj.eq.crm_ny.and.icopy.eq.2
                
                do m=1,crm_nz
                  k = pver-m+1
                  do i=1,ncol

                    trad(i,icopy,k) = t_rad(i,icopy,ii,jj,m) ! used in rad call
                    qvrad(i,icopy,k) = max(1.e-9_r8,qv_rad(i,icopy,ii,jj,m)) ! used in rad call
                    qtot = qc_rad(i,icopy,ii,jj,m) + qi_rad(i,icopy,ii,jj,m) ! temporary var
                    if(qtot.gt.1.e-9) then
                      fice(i,icopy,k) = qi_rad(i,icopy,ii,jj,m)/qtot
                      cld_loc(i,icopy,k) = 0.99_r8
                      cld_crm(i,icopy,ii,jj,m)=0.99_r8
                      cicewp(i,icopy,k) = qi_rad(i,icopy,ii,jj,m)*state%pdel(i,k)/gravit*1000.0 &
                              / max(0.01_r8,cld_loc(i,icopy,k)) ! In-cloud ice water path.
                      cliqwp(i,icopy,k) = qc_rad(i,icopy,ii,jj,m)*state%pdel(i,k)/gravit*1000.0 &
                              / max(0.01_r8,cld_loc(i,icopy,k)) ! In-cloud liquid water path.
                    else
                      fice(i,icopy,k)=0.
                      cld_loc(i,icopy,k)=0.
                      cld_crm(i,icopy,ii,jj,m)=0.
                      cicewp(i,icopy,k) = 0.           ! In-cloud ice water path.
                      cliqwp(i,icopy,k) = 0.           ! In-cloud liquid water path.
                    end if
                    cliqwp_crm(i,icopy,ii,jj,m)=cliqwp(i,icopy,k)
                    cicewp_crm(i,icopy,ii,jj,m)=cicewp(i,icopy,k)
                  end do ! i
                end do ! m

!            Cloud emissivity.
                call cldems(lchnk, ncol, cicewp(:,icopy,:) + cliqwp(:,icopy,:), fice(:,icopy,:), rei, emis(:,icopy,:), cldtau(:,icopy,:))
                do m=1,crm_nz
                  k = pver-m+1
                  do i=1,ncol
                    rel_crm(i,icopy,ii,jj,m)=rel(i,k)
                    rei_crm(i,icopy,ii,jj,m)=rei(i,k)
                    emis_crm(i,icopy,ii,jj,m)=emis(i,icopy,k)
                  end do ! i
                end do ! m

                call cldovrlap(lchnk, ncol, state%pint, cld_loc(:,icopy,:), nmxrgn(:,icopy), pmxrgn(:,icopy,:))
           endif ! use_SPCAM
       ...
       ! Solar radiation computation
           ...
           ! Get aerosol radiative properties.
             ...
             call radcswmx(lchnk, &
#ifdef CRM
               ncol,       pnm,        pbr,        qvrad(:,icopy,:),     o3,         &
#else
               ncol,       pnm,        pbr,        sp_hum,     o3,         &
#endif
               o2_col,     cld_loc(:,icopy,:),        cicewp(:,icopy,:),     cliqwp(:,icopy,:),     rel,        &
               rei,        eccf,       coszrs,     solin,      &
               cam_in%asdir, cam_in%asdif, cam_in%aldir, cam_in%aldif, nmxrgn(:,icopy), &
               pmxrgn(:,icopy,:),     qrs,        qrsc,       fsnt,       fsntc,      fsdtoa, &
               ...
               ! out vars hopefully rewritten on every call
              call t_stopf('rad_sw')
              ...
              if (use_SPCAM) then
                 ...
                 qrs_m(:ncol,icopy,:pver) = qrs_m(:ncol,icopy,:pver) + qrs(:ncol,:pver)*factor_xy
                 qrsc_m(:ncol,:pver) = qrsc_m(:ncol,:pver) + qrsc(:ncol,:pver)*factor_xy
                 do m=1,crm_nz
                    k = pver-m+1
                    qrs_crm(:ncol,icopy,ii,jj,m) = qrs(:ncol,k) / cpair
                 end do
                 do i=1, Nday
                   do k=1, pver
              ...
              ! adding first CRM to buffer
              if (use_SPCAM .and. last_column_crm1) then
                 ...
                 fsnt(:ncol) = fsnt_m(:ncol)
                 fsds(:ncol) = fsds_m(:ncol)
                 qrs(:ncol,:pver) =  qrs_m(:ncol,1,:pver)
                 qrsc(:ncol,:pver) = qrsc_m(:ncol,:pver)
 
              end if

              if (use_SPCAM .and. last_column_crm2) then
               call outfld('CRM_QRS ',qrs_crm(:,2,:,:,:)        ,pcols   ,lchnk   ) ! in cam_diagnostics
              end if 

              if ( (use_SPCAM .and. last_column_crm1) .or. .not. use_SPCAM) then
       ...
       ! Longwave radiation computation
           ...
           if (dolw) then
...
#ifdef CRM
                 lwupcgs, trad(:,icopy,:), qvrad(:,icopy,:), o3, pbr, &
#else
                 lwupcgs, state%t, sp_hum, o3, pbr, &
#endif
                 pnm, state%lnpmid, state%lnpint, n2o, ch4, &
                 cfc11, cfc12, cld_loc(:,icopy,:), emis(:,icopy,:), pmxrgn(:,icopy,:), &
                 nmxrgn(:,icopy), qrl, qrlc, flns, flnt, flnsc, &
              ...
              if (use_SPCAM) then 
                 ...
                 qrl_m(:ncol,icopy,:pver) = qrl_m(:ncol,icopy,:pver) + qrl(:ncol,:pver)*factor_xy
                 qrlc_m(:ncol,:pver) = qrlc_m(:ncol,:pver) + qrlc(:ncol,:pver)*factor_xy
                 do m=1,crm_nz
                    k = pver-m+1
                    qrl_crm(:ncol,icopy,ii,jj,m) = qrl(:ncol,k) / cpair
               end do
              else
                 do i=1,ncol
              ...
              if (use_SPCAM .and. last_column_crm1) then
                 do i=1,ncol
                    ...
                    flns(i) = flns_m(i)
                    flnt(i) = flnt_m(i)
                    qrl(i,:pver) =  qrl_m(i,1,:pver)
                    qrlc(i,:pver) = qrlc_m(i,:pver)
                 end do
              end if
              if (use_SPCAM .and. last_column_crm2) then ! not used at the moment
               call outfld('CRM_QRL ',qrl_crm(:,2,:,:,:)        ,pcols   ,lchnk   )
              end if
       ...
       if (use_SPCAM) then ! SPCAM is not coupled to COSP yet
          ...
          do m=1,crm_nz
            ...
            do i = 1,ncol
            do icopy = 1,crm_ncopy
              crm_qrad(i,icopy,:,:,m) = (qrs_crm(i,icopy,:,:,m)+qrl_crm(i,icopy,:,:,m)) * state%pdel(i,k) ! for energy conservation
            end do
            end do
          end do
          ...
          call cloud_cover_diags_out(lchnk, ncol, cld, state%pmid, nmxrgn(:,1), pmxrgn(:,1,:) ) ! not SPCAM
	   ...
	   !! if counter is the same as cosp_nradsteps, run cosp and reset counter
           if (cosp_nradsteps .eq. cosp_cnt(lchnk)) then
              ...
              call cospsimulator_intr_run(state,  pbuf, &
                   cam_in, emis(:,1,:), coszrs, cliqwp_in=cliqwp(:,1,:), cicewp_in=cicewp(:,1,:)) ! still not SPCAM
...
!DIR$ CONCURRENT
             do i = 1, ncol
                qrs(i,k) = qrs(i,k)/state%pdel(i,k) ! using first CRM
