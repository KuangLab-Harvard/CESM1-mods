module physpkg
  ...
  ! May 2018    N. Kleiner     Added Ding's eddy suppression code
  !-----------------------------------------------------------------------
...
print*,'pvers',pver,pverp
   ...
   call pbuf_add_field('QCWAT_OLD', 'global',dtype_r8,(/pcols,pver/),    QCWAT_oldid     )
   call pbuf_add_field('LCWAT_OLD', 'global',dtype_r8,(/pcols,pver/),    LCWAT_oldid     )
   call pbuf_add_field('TCWAT_OLD', 'global',dtype_r8,(/pcols,pver/),    TCWAT_oldid     )
...
!$OMP PARALLEL DO PRIVATE (C, NCOL, PTEND, FLX_HEAT)
    do c=begchunk, endchunk
       ...
       !call suppress_eddy(phys_state,phys_tend,ztodt)
...
!!!    call suppress_eddy(phys_state,phys_tend,ztodt)
   call replay_correction(phys_state,phys_tend,ztodt)
...
!      type(io_desc_t), pointer :: iodesc,jodesc       !!Added
!      integer, pointer :: tmpfield(:,:)
!     real(r8), allocatable :: tmpfield(:)            !!Added 
...
real(r8) :: msize,mrss
 !fileName = "/n/holylfs04/LABS/kuang_lab/Lab/nedkleiner/MERRAfolder/average.nc"
 !fileName = "/n/kuangdss01/lab/processed_era5/era5_19790429.nc"
 ! fileName = "/n/home01/flamraoui/forcing_merra/MERRA2_0.9x1.25_19800131.nc"
 ! fileName = "/glade/collections/rda/data/ds313.3/0.9x1.25/1980/MERRA2_0.9x1.25_19800131.nc"
...
!then figure out what day it is
  ...
  !---------------------------flamraoui------------------------------
!    fileName = "/n/home08/nedkleiner/holylfs04/MERRAall/IC/MERRA2_19800101.nc"
...
!        iyear=iyear+1
!    endif
!    ndays=ndays-monarray(imonth)
!end do
!
!iday=ndays+monarray(imonth)
